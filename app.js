...
const DEFAULT_STATE={used:[],games:0,words:0,score:0,best:0};
let memoryState={...DEFAULT_STATE,used:[]};

function loadState(){
  try{
    const raw=localStorage.getItem(KEY);
    if(!raw) return {...DEFAULT_STATE,used:[]};

    const parsed=JSON.parse(raw);

    if(!parsed || typeof parsed!=="object") return {...DEFAULT_STATE,used:[]};

    return {
      used:Array.isArray(parsed.used)?parsed.used:[],
      games:Number.isFinite(Number(parsed.games))?Number(parsed.games):0,
      words:Number.isFinite(Number(parsed.words))?Number(parsed.words):0,
      score:Number.isFinite(Number(parsed.score))?Number(parsed.score):0,
      best:Number.isFinite(Number(parsed.best))?Number(parsed.best):0
    };
  }catch(error){
    console.warn("Armazenamento local indisponível ou inválido. Usando memória da sessão.",error);
    return {...memoryState,used:Array.isArray(memoryState.used)?memoryState.used:[]};
  }
}
...
function save(){
  memoryState={
    used:Array.isArray(state.used)?state.used.slice():[],
...
function availableWords(){
  const source=Array.isArray(window.FORGE_WORDS)?window.FORGE_WORDS:[];
  let a=source.filter(x=>
    x &&
    typeof x.normalized==="string" &&
    x.normalized.length>=3 &&
    x.normalized.length<=13 &&
    !state.used.includes(x.normalized)
  );
...
function renderPuzzle(){
 const g=$("#grid");
 if(!g || !current || !current.grid){
   throw new Error("Tabuleiro não encontrado.");
 }
 g.style.gridTemplateColumns=`repeat(${current.grid.length},1fr)`;
 g.innerHTML="";
...
function startGame(){
  try{
    ...
    if(pool.length<10){
      throw new Error("Não há palavras suficientes para iniciar a partida.");
    }

    const chosen=shuffled(pool).slice(0,10);
    const puzzle=makePuzzle(chosen);

    if(!puzzle || !Array.isArray(puzzle.placed) || puzzle.placed.length<1){
      throw new Error("Não foi possível montar o tabuleiro.");
    }
    ...
    if(msg){
      msg.textContent="⚠️ Não foi possível iniciar. Toque em JOGAR novamente.";
    }
  }
}
